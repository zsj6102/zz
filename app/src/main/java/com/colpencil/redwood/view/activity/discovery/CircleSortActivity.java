...
public class CircleSortActivity extends ColpencilActivity implements ICategoryView {
    ...
    protected void initViews(View view) {
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.KITKAT) {
            StatusBarUtil.setColor(this, getResources().getColor(R.color.main_background));
        }
    ...
    void addOnClick() {
        if (SharedPreferencesUtil.getInstance(this).getBoolean(StringConfig.ISLOGIN, false)) {
            ...
            for (int i = 0; i < mineList.size(); i++) {
                ...
                cat_id = cat_id + mineList.get(i).getCat_id() + ",";
            }
            if (stringList.size() <= 0) {
                ToastTools.showShort(this, "至少选择一个分类");
            } else {
                showLoading("正在提交数据中...");
                presenter.saveCircle(cat_id);
            }
        } else {
            ...
            dialog.setListener(new DialogOnClickListener() {
                ...
                public void sureOnClick() {
                    Intent intent = new Intent(CircleSortActivity.this, LoginActivity.class);
