 ...
 */
public class ColBottomNavigation extends FrameLayout {
	...
	private ArrayList<ColBottomNavigationItem> items = new ArrayList<>();
	private ArrayList<View> views = new ArrayList<>();
	private ColBottomNavigationBehavior<ColBottomNavigation> bottomNavigationBehavior;
	 ...
	 * Constructors
	 */
	public ColBottomNavigation(Context context) {
		super(context);
		init(context);
	}

	public ColBottomNavigation(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public ColBottomNavigation(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			ColBottomNavigationItem item = items.get(itemIndex);
			...
			icon.setImageDrawable(ColHelper.getTintDrawable(items.get(i).getDrawable(context),
	...
	private void updateItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				icon.setSelected(true);
				ColHelper.updateTopMargin(icon, inactiveMarginTop, activeMarginTop);
				ColHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
				ColHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
				ColHelper.updateTextSize(title, inactiveSize, activeSize);
				ColHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && colored) {
					...
					ColHelper.updateViewBackgroundColor(this, currentColor,
				...
				icon.setSelected(false);
				ColHelper.updateTopMargin(icon, activeMarginTop, inactiveMarginTop);
				ColHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
				ColHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
				ColHelper.updateTextSize(title, activeSize, inactiveSize);
				ColHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
	...
	private void updateSmallItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				icon.setSelected(true);
				ColHelper.updateTopMargin(icon, inactiveMargin, activeMarginTop);
				ColHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
				ColHelper.updateTopMargin(notification, notificationInactiveMarginTop, notificationActiveMarginTop);
				ColHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
				ColHelper.updateAlpha(title, 0, 1);
				ColHelper.updateWidth(container, notSelectedItemWidth, selectedItemWidth);
				ColHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
						itemInactiveColor, itemActiveColor, forceTint);
				...
				icon.setSelected(false);
				ColHelper.updateTopMargin(icon, activeMarginTop, inactiveMargin);
				ColHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
				ColHelper.updateTopMargin(notification, notificationActiveMarginTop, notificationInactiveMarginTop);
				ColHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
				ColHelper.updateAlpha(title, 1, 0);
				ColHelper.updateWidth(container, selectedItemWidth, notSelectedItemWidth);
				ColHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
						itemActiveColor, itemInactiveColor, forceTint);
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (updateStyle) {
				...
				if (notificationBackgroundDrawable != null) {
					...
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
						notification.setBackground(ColHelper.getTintDrawable(defautlDrawable,
								notificationBackgroundColor, forceTint));
					} else {
						notification.setBackgroundDrawable(ColHelper.getTintDrawable(defautlDrawable,
								notificationBackgroundColor, forceTint));
					}
				}
			}
	 ...
	 * Add an item
	 */
	public void addItem(ColBottomNavigationItem item) {
	 ...
	 * Add all items
	 */
	public void addItems(List<ColBottomNavigationItem> items) {
	 ...
	 */
	public ColBottomNavigationItem getItem(int position) {
	...
	public void setBehaviorTranslationEnabled(boolean behaviorTranslationEnabled) {
		...
		if (getParent() instanceof CoordinatorLayout) {
			...
			if (bottomNavigationBehavior == null) {
				bottomNavigationBehavior = new ColBottomNavigationBehavior<>(behaviorTranslationEnabled);
			} else {
