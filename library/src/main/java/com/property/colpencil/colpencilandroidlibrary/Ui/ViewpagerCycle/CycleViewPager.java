...
public class CycleViewPager extends Fragment implements OnPageChangeListener {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		viewPager = (BaseViewPager) view.findViewById(R.id.viewPager);//图片显示位置
		indicatorLayout = (LinearLayout) view
				.findViewById(R.id.layout_viewpager_indicator);//标识显示位置
		...
		handler = new CycleViewPagerHandler(getActivity()) {
			...
			public void handleMessage(Message msg) {
				...
				if (msg.what == WHEEL && imageViews.size() != 0) {//开始转动
					if (!isScrolling) {
				...
				if (msg.what == WHEEL_WAIT && imageViews.size() != 0) {//停止转动
					handler.removeCallbacks(runnable);
					handler.postDelayed(runnable, time);
				}
			}
